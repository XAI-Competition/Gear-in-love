...
## 附录 A：行识别指纹（全验证集精确 faith；排行榜行无 ID，凭此对号）

排行榜 JSON 只有分数字段、无提交 ID/时间戳（已核验）。识别协议：6 行 macro-F1 逐位相同
（同一分类器）标识本批次；行内按 faith 指纹对号（final2 锚点：迁移误差 ~4e-5）。

| zip | **full-val faith（指纹）** | 与下一名间距 | 批次 |
| --- | ---: | ---: | --- |
| S0a | **0.87472** | 0.0114 | 1 |
| S0b | **0.86330** | 0.0093 | 1 |
| P3 trio_motor_hard | **0.85405** | **0.0010 ⚠️** | 1 |
| P4 mtf_rgbz | **0.85305** | 0.0167 | **2（单独）** |
| P1 quintet_torque | **0.83639** | 0.0102 | 1 |
| P2 quintet_pgby | **0.82615** | — | 1 |

**S0a′ 升级候选（exp-032，可随任意批次上传）**：
`runs/exp032/scale_20k_30ep/wrapped/occ_a0p5_eps0p05_T8/submission.zip` —
faith **0.87943**、macro-F1 **0.96839**（与上表 6 行的 0.98607 不同 → 自带独立指纹，
无识别冲突）。预期总分 ≈0.6215；探针差分参照仍为 S0b，不受影响。

**S0c 最高分候选（exp-035，2026-06-10 终版）**：
`runs/exp035/narrow2_ins30/wrapped/submission.zip` — faith **0.88202**、simplicity
**0.92346**（窄架构 24,48,96 + 插入课程）、macro-F1 **0.96457**（独立指纹，hidden 预期
≈0.915）。**预期总分 ≈0.6455，优先于 S0a′ 上传**。

P3−P4 间距仅 0.0010，低于 0.002 安全线 → **P4 留第二批**（榜单刷新一次后单独上传，
其行即唯一新行，零歧义）。第一批 5 行的最小间距 0.0093，识别余量充足。

## 附录 B：探针本地指标（exp-029 体系 round1b；全部 devkit valid）

| 资产 | full-val faith | proxy audit / low_freq（3000 样本） | model_sha256 前缀 |
| --- | ---: | --- | --- |
| S0a 主候选 | 0.87472 | 0.4224 / 0.6168 | `c6fbb0b52683` |
| S0b 探针参照 | 0.86330 | 0.4470 / 0.6428 | `3de8885950d0` |
| probe_quintet_torque | 0.83639 | 0.4529 / 0.6187 | `100da1a38b87` |
| probe_quintet_pgby | 0.82615 | 0.4452 / 0.6129 | `b54c9b7e060f` |
| probe_trio_motor_hard | 0.85405 | 0.4709 / 0.6667 | `e8733d7a81db` |
| probe_mtf_rgbz | 0.85305 | 0.4456 / 0.6369 | `26466a520aba` |

探针 faith 代价 −0.021～−0.049（harsh 押注的预期损耗）；按锚点换算探针总分仍 ≈0.60
量级——上传无"丢人"风险。完整数字见 `runs/mech_probes_round1b/*/submission.zip` 内
metrics.json（已含全验证集指标）。
