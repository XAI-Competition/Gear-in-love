...
def main() -> int:
    ...
    print(
        f"contexts={len(contexts)} unique_context_windows={len(noisy_indices)} (of {len(y_true)})"
    )
    ...
    result = {
        ...
        "classes_json": str(args.classes_json) if args.classes_json else None,
        "overall": overall,
        "per_class": per_class,
    }
    print(
        ...
        f"stab={overall['relevance_stability']:.4f} "
        f"speed_range=[{overall['estimated_speed_hz_min']}, {overall['estimated_speed_hz_max']}]"
    )
