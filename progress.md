...
- **严谨重测**（跨速**相关性**法，比单 bin argmax 稳健；800 样本/类/速）:
  1. **判别频谱的跨转速相关性**（高=频率结构速度稳定）:

     | 故障 | CTF | MTF | RCF | SWF | BWF | CWF | IRF | ORF |
     | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
     | 谱相关 | 0.01 | 0.12 | 0.11 | -0.01 | 0.09 | 0.01 | 0.08 | 0.08 |

     **全部 ≈ 0** → 判别频率结构**跨转速完全不稳定** → **固定 Hz 频带不可可靠重构**。
     exp-002a 的"清晰频带"是混合转速平均的**假象**，按速拆开即瓦解。路线 A 频带前提**证伪**。
  2. **通道能量分布的跨转速相关性**（高=通道身份速度稳定）:

     | 故障 | CTF | MTF | RCF | SWF | BWF | CWF | IRF | ORF |
     | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
     | 通道相关 | 0.98 | 0.97 | 0.96 | 0.95 | 0.90 | 0.97 | 0.93 | 0.92 |
     | 主导通道 | pgb_y | pgb_y | torque | pgb_y | pgb_z* | motor | torque | motor |

     **0.90–0.98，7/8 类主导通道跨速不变** → **通道身份是唯一可信的、速度不变的机械信号**
     （只取决于传感器位置+故障类型，与转速无关；物理自洽：齿轮箱故障在对应 PGB/扭矩通道更显）。
- **核心结论（订正后）**:
  1. **频带不可重构**（谱相关~0）——路线 A 的频带先验前提**失败**，证伪了 A→B→C 的频带路线。
     这反而强化 exp-002c 的早期判断（100 点单帧窗 + 变速 + 私有频带，频带本地不可得）。
  2. **通道身份可信**（相关 0.90–0.98）——但 **exp-002d 已实测**：用通道先验训练损 faith −0.018，
     换微小且不确定的代理机械收益，**净负**。故"可信"不等于"该用"。
- **产物**: 重写 `src/gearxai_workspace/bands.py` 为诚实的通道-故障关联（带跨速相关证据），
  **不再提供任何虚构的固定 Hz 频带**。
- **对 A→B→C 计划的影响**: 路线 B/C 都依赖"可信频带"，而频带已被证不可重构 →
  **B/C 的频带版本失去地基**。唯一可信的通道杠杆又已被 exp-002d 证明损 faith。
  因此机械对齐**确认为本地不可安全优化**（现在有跨速直接证据，而非仅窗长论证）。
- **教训**: 必须先看脚本真实输出再下结论，绝不搬用旧实验的数字充当新验证。
