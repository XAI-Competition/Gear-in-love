...
## exp-012 — 轻量机械通道先验重导出（Alignment 候选，不重训分类器）

- **日期**: 2026-06-09
- **代码变更**:
  - `ModelConfig.mechanical_prior_strength`：新增推理期 relevance 通道重加权强度，默认 `0.0`，保持原模型完全不变。
  - `MECHANICAL_CHANNEL_PRIOR[9,8]`：温和的类别-通道机械先验，只表达传感器位置偏好，不硬编码具体 Hz 频带。
  - `scripts/export_with_mechanical_prior.py`：从已训练 checkpoint 重导出 ONNX 并评估，不需要重新训练。
  - `evaluate.py`：评估临时目录改到 `runs/_eval_tmp`，避免 Windows 系统 Temp / `.tmp` 权限问题。
- **动机**: Alignment 占 40%，但官方 mechanical band config 私有，本地无法直接测。此前训练期 channel prior 会确定性损失 faithfulness，因此本轮改成**不动分类器、只轻微调整 relevance 输出**，用很小的 simplicity 代价换潜在 hidden mechanical alignment 收益。
- **方法**:
  - 从 `runs/final2/model.pt` 加载最佳 narrow+noise+time-mask 分类器。
  - 仅在 relevance 末尾乘 `exp(strength * log(class_prior_gate))`。
  - 扫 `strength ∈ {0, 0.05, 0.10, 0.20}`，先在 4000 验证样本上看 faith/simplicity，再对 `0.20` 做全验证集打包。
- **命令**:
  ```powershell
  $env:UV_CACHE_DIR = (Resolve-Path .\.uv-cache-local).Path
  uv run --no-sync python scripts\export_with_mechanical_prior.py `
    --checkpoint runs\final2\model.pt --out runs\mechanical_prior_s020_full\model.onnx `
    --strength 0.20 --eval-n -1
  uv run --no-sync gearxai package `
    --model runs\mechanical_prior_s020_full\model.onnx `
    --data-dir data\prepared --split validation `
    --out runs\mechanical_prior_s020_full\submission.zip
  ```
- **4000 样本 sweep**:

  | strength | macro-F1 | faith | simplicity | ops |
  | ---: | ---: | ---: | ---: | ---: |
  | 0.00 | 0.9889 | 0.7323 | 0.9220 | 41 |
  | 0.05 | 0.9889 | 0.7326 | 0.9127 | 52 |
  | 0.10 | 0.9889 | 0.7329 | 0.9127 | 52 |
  | 0.20 | 0.9889 | 0.7335 | 0.9127 | 52 |

- **全验证集结果** (`runs/mechanical_prior_s020_full/submission.zip`, 83,790 样本):

  | 模型 | macro-F1 | faith | simplicity | expl_partial |
  | --- | ---: | ---: | ---: | ---: |
  | final2 | 0.9893 | 0.7325 | 0.9220 | 0.4774 |
  | mechanical_prior_s020 | 0.9893 | 0.7336 | 0.9127 | 0.4760 |

- **产物**:
  - `runs/mechanical_prior_s020_full/model.onnx`
  - `runs/mechanical_prior_s020_full/submission.zip`
  - `model_sha256`: `f5cd2296ec1a9eafa143748319a3a07c86c0973edcfba0a084fa3c64dd995488`
- **结论**:
  - 本地可测指标上，通道先验没有伤分类，faith 还小幅上涨 `+0.0011`；simplicity 因多 11 个 ONNX 算子下降 `-0.0093`，所以 `expl_partial` 小幅低于 final2。
  - 这个模型不是本地可测最优，而是 **Alignment 探索候选**：如果 hidden mechanical alignment 对类别-通道传感器偏好敏感，`mechanical_prior_s020` 可能优于 final2；如果 alignment 收益不明显，final2 仍是更稳的本地最优提交。
  - 下一步若有 public/official 提交通道，应并行提交或对比 `final2` 与 `mechanical_prior_s020`；没有 leaderboard 反馈时，保守选择仍是 `final2`，冲 Alignment 可选 `mechanical_prior_s020`。

---

## exp-013 — 工况/错误分析 + Relevance 可视化 + 更强 Alignment sweep

- **日期**: 2026-06-09
- **目标**: 补足“模型为什么错、错在哪些工况、解释图怎么变”的分析工作量，并继续探索 exp-012 的通道先验强度边界。
- **新增脚本**:
  - `scripts/analyze_predictions.py`: 对 ONNX 预测做完整验证集分析，输出 confusion matrix、per-class F1、per-condition / per-speed-group 指标、主要错误对。
  - `scripts/plot_relevance_examples.py`: 导出信号 + relevance heatmap，对比 `final2` 与机械先验版本。
- **分类分析产物**:
  - `runs/analysis_final2/`
  - `runs/analysis_mechanical_prior_s020/`
  - 由于机械先验只改 relevance，不改概率，两个版本分类结果一致：83,790 验证样本，macro-F1 **0.9893**，errors **894**。
- **关键错误分析**:

  | 维度 | 发现 |
  | --- | --- |
  | 最弱类别 | HEA F1 0.9759、MTF F1 0.9828、ORF F1 0.9846、IRF F1 0.9851 |
  | 最强类别 | BWF F1 1.0000、SWF 真类召回 1.0000 |
  | 最弱工况组 | variable_speed macro-F1 **0.9821**，占 793/894 个错误 |
  | 固定转速 | 20Hz 0.9891、30Hz 0.9985、40Hz 0.9971、50Hz 0.9998 |
  | 最弱具体工况 | `PGB_Variable_speed_Experiment2` 0.9436、Experiment6 0.9477、Experiment9 0.9701 |
  | 主要错误对 | IRF→ORF 244、MTF→HEA 217、RCF→HEA 145、RCF→SWF 57 |

- **解释图可视化**:
  - 产物: `runs/relevance_examples_final2_vs_mech/`
  - 生成 8 张 PNG：3 个正确样本 + 5 个错误样本；每张图包含 8 通道输入信号、`final2` relevance、mechanical-prior relevance。
  - 用途: 直观看通道先验是否把解释图从纯幅值热点推向更符合故障类别的传感器通道。
- **更强机械先验 sweep**（4000 验证样本，仍不重训分类器）:

  | strength | macro-F1 | faith | deletion↓ | insertion↑ | simplicity |
  | ---: | ---: | ---: | ---: | ---: | ---: |
  | 0.00 | 0.9889 | 0.7323 | 0.243 | 0.708 | 0.9220 |
  | 0.20 | 0.9889 | 0.7335 | 0.242 | 0.709 | 0.9127 |
  | 0.30 | 0.9889 | 0.7340 | 0.241 | 0.709 | 0.9127 |
  | 0.50 | 0.9889 | 0.7352 | 0.239 | 0.709 | 0.9127 |
  | 0.80 | 0.9889 | 0.7365 | 0.236 | 0.709 | 0.9127 |
  | 1.00 | 0.9889 | 0.7371 | 0.234 | 0.708 | 0.9127 |
  | 1.50 | 0.9889 | 0.7379 | 0.229 | 0.705 | 0.9127 |

- **全验证集新候选** (`strength=1.0`):

  | 模型 | macro-F1 | faith | simplicity | expl_partial |
  | --- | ---: | ---: | ---: | ---: |
  | final2 | 0.9893 | 0.7325 | 0.9220 | 0.4774 |
  | mechanical_prior_s020 | 0.9893 | 0.7336 | 0.9127 | 0.4760 |
  | **mechanical_prior_s100** | **0.9893** | **0.7364** | **0.9127** | **0.4771** |

- **命令**:
  ```powershell
  uv run --no-sync python scripts\analyze_predictions.py `
    --model runs\final2\model.onnx --out-dir runs\analysis_final2
  uv run --no-sync python scripts\plot_relevance_examples.py `
    --model-a runs\final2\model.onnx `
    --model-b runs\mechanical_prior_s020_full\model.onnx `
    --label-a final2 --label-b mech_s020 `
    --out-dir runs\relevance_examples_final2_vs_mech
  uv run --no-sync python scripts\export_with_mechanical_prior.py `
    --checkpoint runs\final2\model.pt `
    --out runs\mechanical_prior_s100_full\model.onnx `
    --strength 1.00 --eval-n -1
  uv run --no-sync gearxai package `
    --model runs\mechanical_prior_s100_full\model.onnx `
    --data-dir data\prepared --split validation `
    --out runs\mechanical_prior_s100_full\submission.zip
  ```
- **产物**:
  - `runs/mechanical_prior_s100_full/submission.zip`
  - `model_sha256`: `6627ae449c9ab0dc254c1d9aa1aa1be62cb9777e4b1a4852f9227e1e6bfd3aef`
- **结论 / 下一步**:
  - 分类瓶颈主要在 **variable-speed 工况**，尤其 Experiment2/6；若继续训练，方向应是更针对变速工况的增强/分组验证，而不是固定转速。
  - Alignment 先验强度从 0.2 提到 1.0 后，faith 全量 **0.7325→0.7364**，simplicity 固定为 0.9127，`expl_partial` 几乎追平 final2，同时更可能提升 hidden mechanical alignment。
  - `strength=1.5` 在子集上 faith 仍涨，但 insertion 已开始下降，说明过强通道集中可能进入风险区；当前推荐 **mechanical_prior_s100** 作为“冲 Alignment”的更优候选，`final2` 仍是 simplicity 更高的保守候选。

---

## exp-014 — 变速工况过采样专项训练（负结果：简单提高变速比例无效）

- **日期**: 2026-06-09
- **目标**: exp-013 发现 894 个错误中 793 个来自 `variable_speed`，因此尝试在训练子集中提高变速工况比例，验证能否改善最弱的 variable-speed Experiment2/6。
- **代码变更**:
  - `data.py`: 新增 `load_metadata()` 与 `balanced_subset_indices_with_variable_fraction()`，支持按类别抽样时指定每类 variable-speed 比例。
  - `TrainConfig.train_variable_fraction` 与 CLI 参数 `--train-variable-fraction`，默认 `None`，不影响原 final2 复现。
- **实验设计**:
  - 保持 final2 架构与增强：narrow + `noise_std=0.1` + `time_mask_frac=0.15`。
  - 训练规模不变：`train_per_class=40000`，总 360k 窗口，45 epoch，RTX 4060。
  - 对照两档变速比例：`0.65`（温和）与 `0.80`（激进）。
- **命令**:
  ```powershell
  uv run --no-sync python scripts\train_baseline.py `
    --train-per-class 40000 --train-variable-fraction 0.65 `
    --val-per-class 4000 --epochs 45 --batch-size 768 --device cuda `
    --noise-std 0.1 --time-mask-frac 0.15 `
    --out runs\varboost_065\model.onnx
  uv run --no-sync python scripts\train_baseline.py `
    --train-per-class 40000 --train-variable-fraction 0.80 `
    --val-per-class 4000 --epochs 45 --batch-size 768 --device cuda `
    --noise-std 0.1 --time-mask-frac 0.15 `
    --out runs\varboost_080\model.onnx
  ```
- **完整验证集分类结果**:

  | 模型 | train variable fraction | macro-F1 | errors | variable-speed macro-F1 | variable errors |
  | --- | ---: | ---: | ---: | ---: | ---: |
  | final2 | random (~0.53) | **0.9893** | **894** | **0.9821** | **793** |
  | varboost_065 | 0.65 | 0.9844 | 1302 | 0.9740 | 1143 |
  | varboost_080 | 0.80 | 0.9833 | 1394 | 0.9734 | 1169 |

- **4000 样本 devkit 快评**:

  | 模型 | macro-F1 | faith | simplicity |
  | --- | ---: | ---: | ---: |
  | varboost_065 | 0.9867 | 0.7237 | 0.9220 |
  | varboost_080 | 0.9832 | 0.7266 | 0.9220 |

- **关键工况对比**:

  | condition | final2 macro-F1 | varboost_065 | varboost_080 |
  | --- | ---: | ---: | ---: |
  | `PGB_Variable_speed_Experiment2` | 0.9436 | 不在最差前 10（有改善迹象） | 不在最差前 10（有改善迹象） |
  | `PGB_Variable_speed_Experiment6` | 0.9477 | 0.9458 | 0.9519 |
  | `PGB_Variable_speed_Experiment9` | **0.9701** | **0.9069** | **0.8827** |

- **错误模式变化**:
  - final2 最大错误对：IRF→ORF 244。
  - varboost_065: IRF→ORF **523**，HEA→ORF 144，IRF→CWF 109。
  - varboost_080: IRF→ORF **651**，RCF→HEA 171，HEA→MTF 167。
- **结论**:
  - **否决简单变速过采样**。它没有解决 variable-speed 总体瓶颈，反而显著恶化 Experiment9 和 IRF/ORF 边界；整体 macro-F1 与 faith 都低于 final2。
  - 这说明问题不是“变速样本数量不够”，而是不同 variable-speed 实验之间存在分布差异；粗暴提高变速比例会让模型偏向某些变速模式，牺牲其他变速实验。
  - 下一步若继续优化变速工况，应转向更细粒度方案：按 `experiment_id` 做分组诊断/留一验证、针对 Experiment2/6/9 的信号统计差异分析，或做 time-warp / speed-jitter 这类模拟变速形态的增强，而不是采样比例过采样。

---

## exp-015 — Time-warp 变速形态增强小规模 sweep（负结果：不改善 variable-speed）

- **日期**: 2026-06-09
- **目标**: exp-014 证明过采样无效后，尝试更贴近变速物理形态的增强：训练时对窗口做轻微时间拉伸/压缩，再插值回 100 点，模拟转速变化导致的波形尺度变化。
- **代码变更**:
  - `TrainConfig.time_warp_std` 与 CLI `--time-warp-std`。
  - `random_time_warp()`：每个 batch 内按样本采样 log-normal 时间尺度，范围 clamp 到 `[0.85, 1.18]`，线性插值回原长度。默认 `0.0`，不影响已有模型。
- **实验设计**:
  - 小规模快速判断，不直接上全量：`train_per_class=10000`，90k 训练窗口，20 epoch，RTX 4060。
  - 固定 final2 的有效增强：`noise_std=0.1`、`time_mask_frac=0.15`。
  - 对照 `time_warp_std ∈ {0, 0.03, 0.06}`。
- **命令**:
  ```powershell
  uv run --no-sync python scripts\train_baseline.py `
    --train-per-class 10000 --val-per-class 4000 --epochs 20 `
    --batch-size 768 --device cuda --noise-std 0.1 --time-mask-frac 0.15 `
    --time-warp-std 0.03 --out runs\timewarp_003_sweep\model.onnx
  uv run --no-sync python scripts\train_baseline.py `
    --train-per-class 10000 --val-per-class 4000 --epochs 20 `
    --batch-size 768 --device cuda --noise-std 0.1 --time-mask-frac 0.15 `
    --time-warp-std 0.06 --out runs\timewarp_006_sweep\model.onnx
  ```
- **完整验证集分类分析**:

  | 模型 | time_warp_std | macro-F1 | errors | variable-speed macro-F1 | variable errors |
  | --- | ---: | ---: | ---: | ---: | ---: |
  | timewarp_000_sweep | 0.00 | **0.9594** | **3392** | **0.9560** | **1958** |
  | timewarp_003_sweep | 0.03 | 0.9510 | 4090 | 0.9396 | 2658 |
  | timewarp_006_sweep | 0.06 | 0.9542 | 3820 | 0.9482 | 2269 |

- **4000 样本 devkit 快评**:

  | 模型 | macro-F1 | faith | simplicity |
  | --- | ---: | ---: | ---: |
  | timewarp_000_sweep | **0.9607** | 0.7197 | 0.9220 |
  | timewarp_003_sweep | 0.9510 | **0.7204** | 0.9220 |
  | timewarp_006_sweep | 0.9553 | 0.7121 | 0.9220 |

- **关键发现**:
  - time-warp 没有改善 variable-speed，`0.03` 和 `0.06` 都低于同规模无 time-warp 对照。
  - `0.03` 的 faith 与对照接近，但分类和 variable-speed 都明显更差；`0.06` faith 也下降。
  - 典型失败：`timewarp_003_sweep` 下 `PGB_Variable_speed_Experiment9` macro-F1 仅 0.8296，远低于同规模对照和 final2。
- **结论**:
  - **否决当前 time-warp 实现**，不做全量训练。100 点窗口太短，直接拉伸/压缩可能破坏已有局部故障模式，收益不足以抵消分类损失。
  - 变速问题仍然存在，但下一步不应继续粗暴时间缩放；更可能有价值的是按 variable-speed experiment 做信号统计差异分析，或使用更保守的测试时/解释时处理，而非训练期强增强。

---

## exp-016 — 简化 mechanical_prior_s100 的 ONNX 图（正结果：保留 faith，追回 simplicity）

- **日期**: 2026-06-09
- **目标**: exp-013 的 `mechanical_prior_s100` 是当前最有希望的 Alignment 候选，但通用实现用了 `log/exp`，ONNX 算子从 41 增到 52，simplicity 从 0.9220 降到 0.9127。对于 `strength=1.0`，数学上不需要 `exp(log(x)*strength)`，可直接乘 `prior_gate`。
- **代码变更**:
  - `model.py`: 当 `mechanical_prior_strength == 1.0` 时跳过 `log/exp/clamp` 路径，直接 `relevance *= prior_gate`。
  - 不改变分类器、不改变 `strength=1.0` 的 relevance 数学结果，只减少 ONNX 图复杂度。
- **命令**:
  ```powershell
  uv run --no-sync python scripts\export_with_mechanical_prior.py `
    --checkpoint runs\final2\model.pt `
    --out runs\mechanical_prior_s100_lean\model.onnx `
    --strength 1.0 --eval-n -1
  uv run --no-sync gearxai package `
    --model runs\mechanical_prior_s100_lean\model.onnx `
    --data-dir data\prepared --split validation `
    --out runs\mechanical_prior_s100_lean\submission.zip
  ```
- **完整验证集结果**:

  | 模型 | macro-F1 | faith | simplicity | ops | expl_partial |
  | --- | ---: | ---: | ---: | ---: | ---: |
  | final2 | 0.9893 | 0.7325 | **0.9220** | 41 | 0.4774 |
  | mechanical_prior_s100 | 0.9893 | 0.7364 | 0.9127 | 52 | 0.4771 |
  | **mechanical_prior_s100_lean** | **0.9893** | **0.7364** | **0.9178** | **46** | **0.4781** |

- **产物**:
  - `runs/mechanical_prior_s100_lean/submission.zip`
  - `model_sha256`: `ed0e2f4ed64e80f20f873ac09b7bc68b457806cd37b6d1997dbbd5e039642fee`
- **结论**:
  - **正结果**：不改变预测与 faith 的情况下，simplicity 从 0.9127 回升到 0.9178；本地可测 `expl_partial` 达到 **0.4781**，略高于 final2 的 0.4774。
  - 当前最佳“冲 Alignment / 解释性”候选更新为 **`runs/mechanical_prior_s100_lean/submission.zip`**。如果只追求最简单保守模型，final2 仍可用；如果没有官方反馈、但想提高综合解释性，lean s100 是更合理的单提交选择。

---

## exp-017 — 移除恒等 channel gate + 保留 s100 机械先验（正结果：本地可测全面超过 final2）

- **日期**: 2026-06-10
- **目标**: 继续沿 exp-016 的低风险路径优化 ONNX 简洁度。检查发现 final2 的 `channel_gate` 是零初始化且未被训练的近似恒等门控，但导出后仍贡献 `Gemm/Add/Softplus/Mul` 等节点。若使用推理期机械先验，原恒等 gate 可以移除。
- **代码变更**:
  - `scripts/export_with_mechanical_prior.py` 新增 `--disable-channel-attention`。
  - 该模式用 `ModelConfig(channel_attention=False, mechanical_prior_strength=1.0)` 重建模型，并用 `strict=False` 加载 final2 checkpoint，跳过未使用的 `channel_gate.*` 权重。
  - 分类器权重不变；relevance 中只保留 `forward Grad-CAM × |x| × mechanical_prior_s100`。
- **命令**:
  ```powershell
  uv run --no-sync python scripts\export_with_mechanical_prior.py `
    --checkpoint runs\final2\model.pt `
    --out runs\mechanical_prior_s100_nogate\model.onnx `
    --strength 1.0 --disable-channel-attention --eval-n -1
  uv run --no-sync gearxai package `
    --model runs\mechanical_prior_s100_nogate\model.onnx `
    --data-dir data\prepared --split validation `
    --out runs\mechanical_prior_s100_nogate\submission.zip
  ```
- **完整验证集结果**:
  ...
  | final2 | 0.9893 | 0.7325 | 0.9220 | 41 | 0.4774 |
  | mechanical_prior_s100_lean | 0.9893 | 0.7364 | 0.9178 | 46 | 0.4781 |
  | **mechanical_prior_s100_nogate** | **0.9893** | **0.7364** | **0.9238** | **39** | **0.4793** |

- **产物**:
  - `runs/mechanical_prior_s100_nogate/submission.zip`
  - `model_sha256`: `d079aabd12ca47f6d9365cade3380fe831f79b3b46f7a59f4c4a61d6b5745f09`
- **结论**:
  - **正结果，当前最佳**：不重训、不改分类器，直接把原本无贡献的恒等 channel gate 移除，既保留 `mechanical_prior_s100` 的 faith 提升，又让 simplicity 超过 final2。
  - 本地可测 `expl_partial` 从 final2 的 **0.4774** 提到 **0.4793**；同时保留对 hidden mechanical alignment 的潜在收益。
  - 当前单提交首选更新为 **`runs/mechanical_prior_s100_nogate/submission.zip`**。
